...
describe('GeminiConversationAdapter', () => {
  ...
  beforeEach(async () => {
    ...
    const mockTracingService = {
      withSpan: jest.fn().mockImplementation((_name: string, fn: (span: unknown) => Promise<unknown>) =>
        fn({ setAttribute: jest.fn(), setStatus: jest.fn(), recordException: jest.fn(), end: jest.fn() }),
      ),
      traceAICall: jest.fn().mockImplementation((_p: unknown, _m: unknown, _o: unknown, fn: () => Promise<unknown>) => fn()),
      traceVectorSearch: jest.fn().mockImplementation((_q: unknown, fn: () => Promise<unknown>) => fn()),
      traceDBOperation: jest.fn().mockImplementation((_o: unknown, _c: unknown, fn: () => Promise<unknown>) => fn()),
    };

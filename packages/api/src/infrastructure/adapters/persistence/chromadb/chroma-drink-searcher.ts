 ...
 * flexibility in the embedding strategy (Gemini, OpenAI, local models, etc.)
 */
@Injectable()
export class ChromaDrinkSearcher implements IDrinkSearcherPort, OnModuleInit {
  ...
  async findSimilar(
    ...
    if (cached) {
      ...
      // Reconstitute Drink entities from cached plain objects
      return cached.map((result) => ({
        drink: this.reconstituteDrinkFromCache(result.drink),
        score: result.score,
      }));
    }

    // Cache miss - query ChromaDB with timing and tracing
    const startTime = Date.now();
    const whereClause = this.buildWhereClause(filters);

    const mappedResults = await this.tracingService.traceVectorSearch(query, async () => {
    ...
    this.metricsService.recordVectorSearch(this.collectionName, durationSec);
  ...
  private metadataToDrink(id: string, metadata: Metadata): Drink {
    return Drink.reconstitute({
      ...
      isHot: metadata.isHot !== undefined ? Boolean(metadata.isHot) : true,
      imageUrl: metadata.imageUrl !== undefined ? String(metadata.imageUrl) : '',
    });
  }

  /**
   * Reconstitutes a Drink entity from a cached plain object.
   * When objects are stored in Redis cache, they lose their class methods.
   * This method recreates proper Drink entities from the cached data.
   */
  private reconstituteDrinkFromCache(cachedDrink: unknown): Drink {
    const d = cachedDrink as {
      id: { value: string };
      name: string;
      description: string;
    ...
    return Drink.reconstitute({
      id: DrinkId.fromString(d.id.value),
      name: d.name,
      description: d.description,
      basePrice: Money.fromCents(d.basePrice.cents, d.basePrice.currency),
      customizationOptions: new CustomizationOptions(
        d.customizationOptions.milk,
        d.customizationOptions.syrup,
        d.customizationOptions.sweetener,
        d.customizationOptions.topping,
        d.customizationOptions.size,
      ),
      isHot: d.isHot,
      imageUrl: d.imageUrl ?? '',
    });

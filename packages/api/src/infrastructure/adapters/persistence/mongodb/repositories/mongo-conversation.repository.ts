...
export class MongoConversationRepository implements IConversationRepositoryPort {
  ...
  async save(conversation: Conversation): Promise<void> {
    try {
      ...
      if (exists) {
        ...
        this.metricsService.recordDBQuery('update', this.collectionName, durationSec);
        ...
        // Record metrics
        const durationSec = (Date.now() - startTime) / 1000;
        this.metricsService.recordDBQuery('create', this.collectionName, durationSec);
  ...
  async getRecentHistory(id: ConversationId, limit = 10): Promise<Conversation | null> {
    ...
    const durationSec = (Date.now() - startTime) / 1000;
    this.metricsService.recordDBQuery('aggregate', this.collectionName, durationSec);

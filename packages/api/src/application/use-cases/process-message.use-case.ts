...
import { ConversationId, Money, OrderItem } from '@domain/value-objects';
import {
  // Outbound ports
  IConversationAIPort,
...
import { mapDrinkToCard } from './helpers/drink-action-helpers';
...
export class ProcessMessageUseCase implements IProcessMessagePort {
  ...
  async execute(
    ...
    try {
      ...
      // Fallback: If intent is order_drink but no extracted order, try to extract from message
      let extractedOrders = aiResponse.extractedOrders;
      let extractedOrder = aiResponse.extractedOrder;

      const soundsLikeConfirmation = /\bagregu[eé]|\bañad[íi]|\bsum[eé]|\blo agregu[eé]/i.test(aiResponse.message ?? '');
      if ((aiResponse.intent === 'order_drink' || soundsLikeConfirmation) && !extractedOrders?.items?.length && !extractedOrder) {
        this.logger.warn('Intent is order_drink but no extracted order - attempting fallback extraction');
        const fallbackOrder = await this.conversationAI.extractOrderFromMessage(
          input.message,
          relevantDrinks,
        );
        if (fallbackOrder) {
          extractedOrder = fallbackOrder;
          this.logger.debug(`Fallback extraction found: ${fallbackOrder.drinkName}`);
        }
      }
      ...
      const { message: augmentedMessage, cards } = await this.handleSpecialActions(
        aiResponse.message,
  ...
  private async handleOrderIntent(
    ...
    // Record order creation metric (only for new orders)
    if (!currentOrder) {
      this.metricsService.recordOrder('created');
    }
  ...
  private async handleConfirmIntent(
    ...
    try {
      if (currentOrder.canBeConfirmed()) {
        ...
        await this.orderRepository.saveWithConversation(currentOrder, conversation.id.toString());
        this.metricsService.recordOrder('confirmed');
  ...
  private async handleCancelIntent(
    ...
    try {
      currentOrder.cancel();
      await this.orderRepository.saveWithConversation(currentOrder, conversation.id.toString());
      this.metricsService.recordOrder('cancelled');
      conversation.clearCurrentOrder();
  ...
  private translateDrinkName(name: string): string {
    const translations: Record<string, string> = {
      ...
      'frappuccino de caramelo': 'caramel frappuccino',
      'frappuccino de mocha': 'mocha frappuccino',
    };
  ...
  private buildOutput(
    ...
    const currentOrder = order ? buildOrderSummary(order) : null;
    const suggestedReplies = getSuggestedActions(order);
  ...
  private async handleSpecialActions(
  ...
  ): Promise<{ message: string; cards?: DrinkCardDto[] }> {
    ...
    if (searchAction && searchAction.payload?.query) {
      const rawFilters = searchAction.payload.filters as { isIced?: boolean; maxPrice?: number } | undefined;
   ...
   */
  private async handleGetFullMenu(
    originalMessage: string,
  ): Promise<{ message: string; cards?: DrinkCardDto[] }> {
    try {
      ...
      const cards: DrinkCardDto[] = allDrinks.map((d) => mapDrinkToCard(d));
      return { message: originalMessage, cards };
    } catch (error) {
  ...
  private async handleGetDrinkDetails(
    ...
    try {
      ...
      const cards: DrinkCardDto[] = [mapDrinkToCard(drink)];
      return { message: originalMessage, cards };
    } catch (error) {
  ...
  private async handleSearchDrinks(
    originalMessage: string,
    query: string,
    filters?: { isHot?: boolean; maxPrice?: number },
  ): Promise<{ message: string; cards?: DrinkCardDto[] }> {
    try {
      const searchFilters = filters ? {
        isHot: filters.isHot,
        maxPrice: filters.maxPrice ? Money.fromCents(filters.maxPrice * 100, 'USD') : undefined,
      } : undefined;

      const searchResults = await this.drinkSearcher.findSimilar(query, 5, searchFilters);
      ...
      const cards: DrinkCardDto[] = searchResults.map((r) =>
        mapDrinkToCard(r.drink, r.score),
      );
      return { message: originalMessage, cards };
    } catch (error) {
